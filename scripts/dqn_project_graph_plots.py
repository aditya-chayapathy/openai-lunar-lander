...
class DQN(object):
    ...
    def eval(self, save_snapshot=True):
        ...
        if self.num_episodes % 500 == 0:
            self.eps_array.append(self.eps_start)
            self.rewards_array.append(total_reward)
            plt.plot(self.eps_array, self.rewards_array)
            plt.xlabel("Epsilon")
            plt.ylabel("Episode Value")
            plt.savefig("e_vs_episodeValue.jpg")
